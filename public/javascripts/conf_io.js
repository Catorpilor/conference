(function($){
    ...
    tmplt = {
        user: [
            ...
            '<p class="actions"><button id="bmute" type="${mem_type}" mem-id="${mem_id}" act="mute" class="btn btn-info">静音</button><button id="bkick" mem-id="${mem_id}" type="${mem_type}" class="btn btn-danger">离开</button></p>',
            '</div>',
            '</div>',
            '</div>'
        ].join(""),
    ...
    function bindSocketEvent(confid){
        ...
        //on conference status
        socket.on(confid+'-status',function(data) {
            console.log(data.ctime);
            $('.stimer').text(data.ctime);
            console.log($('.stimer').text());
            //update the button
    ...
    $(function(){
        ...
        var fileprefix = $('.fileprefix').text();
        ...
        $('.col-md-6 #arecord').on('click',function(){
            var filepath = fileprefix+$(".stimer").text()+'_'+confid+'.wav';
        ...
        $('.col-md-6 #record').on('click',function(){
            if($(this).attr('act')=='srecord'){
                ...
                data.ctime = curTime.getTime();
                data.otime = $('.oritimer').text();
                data.confid = confid;
                $(".stimer").html(curTime.getTime());
                $(this).blur();
                $.ajax({
                    type: 'POST',
                ...
                if(confirm('确认停止对当前会议室录音?')){
                    var datafile = fileprefix+$(".stimer").text()+'_'+confid+'.wav';
                    $(this).attr('act','srecord');
                    $(this).attr('value','开始录音');
                    //$(this).addClass('hide');
                    $('.col-md-6 #arecord').addClass('hide');
                    $(this).blur();
        ...
        $modal.on('click', '.confirm', function(){
            ...
            if(destnumber.length >= 3 ){
                ...
                $modal.modal('hide');
                $('#framedisplay').prepend('<div class="alert alert-info fade in" mid="invite">' +
                '邀请已发送！<button type="button" class="close" data-dismiss="alert">x</button>' +'</div>');
                setTimeout(function(){
                    $('div[mid="invite"]').alert('close');
                },3500);
            }else{
                // $('#mymodal .modal-dialog').prepend('<div class="alert alert-error fade in">' +
                // '号码格式不正确！<button type="button" class="close" data-dismiss="alert">x</button>' +'</div>');
                alert('号码不正确!请重新输入');
                $('#fordestnum').focus();
                $(this).blur();
            }
